...
func TryGenMake() (err os.Error) {
	if GenMake {
		...
		if genBuild {
			...
			_, err = fmt.Fprintf(buildFile, "if [ \"$1\" = \"goinstall\" ]; then\n")
			_, err = fmt.Fprintf(buildFile, "echo Running goinstall \\\n")
			for gp := range gm {
				if _, ok := Packages[gp]; ok {
					continue
				}
				gp = strings.Trim(gp, "\"")
				_, err = fmt.Fprintf(buildFile, "&& echo \"goinstall %s\" \\\n", gp)
				_, err = fmt.Fprintf(buildFile, "&& goinstall %s \\\n", gp)
			}
			_, err = fmt.Fprintf(buildFile, "\n")
			_, err = fmt.Fprintf(buildFile, "else\n")
			_, err = fmt.Fprintf(buildFile, "echo Building \\\n")
			for _, pkg := range ListedPkgs {
				pkg.AddToBuild(buildFile)
			}
			_, err = fmt.Fprintf(buildFile, "\n")
			_, err = fmt.Fprintf(buildFile, "fi\n")
